...
3. **SG11 QoL** (aliases/tmux/SSH deploy scripts) + **SG12 docs** (post-install guide, Zen Coding, per-project workflow, propagating-fixes) + **SG13 E2E validation runbook**.
4. **Deferred / human-required:** VRAM expansion (only for 70B+), tenant-user creation (sudo), survives-reboot checks, BIOS changes, cursor `distrobox-export --app` (touches host desktop).
