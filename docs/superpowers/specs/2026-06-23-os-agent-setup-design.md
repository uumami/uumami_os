...
2. **SG10.5 tenant model:** `tenant-create` + `work` against the SG8 interfaces + SG4 registry schema (Tier 2a default; creating tenant Linux users = sudo = human-required; isolation logic sandbox-proven first).
3. **SG11 QoL** — implement the deploy scripts (shared-aliases / tmux `.tmux.conf` + auto-attach / SSH) named in the SG8 table; docs/Zen/workflow already written in SG12.
4. **SG13 E2E validation runbook** — the human-in-the-loop sequence (recreate os_agent, create two tenants, confirm mutual unreadability, GPU/reboot survival).
5. **Deferred / human-required:** os_agent recreate, VRAM expansion (only for 70B+), tenant-user creation (sudo), survives-reboot checks, BIOS changes, cursor `distrobox-export --app` (touches host desktop).
